...
class TrackAwarePoseHungarianAssigner(PoseHungarianAssigner):
    ...
    def assign(self,
        ...
        assigned_gt_inds = kpt_pred.new_full((num_kpts,), -1, dtype=torch.long)
        assigned_labels = kpt_pred.new_full((num_kpts,), -1, dtype=torch.long)
        if num_gts == 0 or num_kpts == 0:
            if num_gts == 0:
                assigned_gt_inds[:] = 0
            self.n_hard_matched = 0
...
@BBOX_ASSIGNERS.register_module()
class PoseHungarianAssignerV10(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth.
    ...
    def assign(self,
        ...
        Returns:
            :obj:`AssignResult`: The assigned result.
        """
        # 每个样本gt的个数，预测结果的个数 eg: gt:5个， pred:300个
        num_gts, num_kpts = gt_keypoints.size(0), kpt_pred.size(0)

        # 1. assign -1 by default 默认设置纬-1， 表示gt与预测均不匹配, shape: (300,)
        assigned_gt_inds = kpt_pred.new_full((num_kpts, ),
                                             -1,
                                             dtype=torch.long)
        assigned_labels = kpt_pred.new_full((num_kpts, ),
        ...
        # 2. compute the weighted costs
        # classification cost 计算每个gt与预测值的误差值
        cls_cost = self.cls_cost(cls_pred, gt_labels) # shape: 300, 5

        # 计算标准化流损失
        # keypoint regression RLE cost
        gt_keypoints_reshape = gt_keypoints.reshape(gt_keypoints.shape[0], -1,
                                                    3)
        # gt中关节点是否可见 shape: 5, 15 ---> 5个gt, 15个关节点
        valid_kpt_flag = gt_keypoints_reshape[..., -1]
        
        # 剥离计算图，防止影响梯度计算
        kpt_pred_tmp = kpt_pred.clone().detach().reshape(
            kpt_pred.shape[0], -1, 2)
        sigmas_pred = sigmas_pred.clone().detach()
        
        # 将gt中的关节点坐标归一化到0-1之间，shape：1，5，15，2
        normalize_gt_keypoints = gt_keypoints_reshape[
            ..., :2] / factor[:, :2].unsqueeze(0)
        
        kpt_cost = self.kpt_cost(kpt_pred_tmp, sigmas_pred, normalize_gt_keypoints,
                                 valid_kpt_flag, flow_model)
        # # keypoint OKS cost
        # kpt_pred_tmp = kpt_pred.clone().detach().reshape(
        #     kpt_pred.shape[0], -1, 2)
        # kpt_pred_tmp = kpt_pred_tmp * factor[:, :2].unsqueeze(0)
