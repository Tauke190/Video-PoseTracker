...
def main():
    ...
    if not distributed:
        ...
        # # backbone
        # input_tensor = torch.ones((1, 3, 3, 512, 512)).float().to('cuda:0')
        # # neck
        # # head.transformer.encoder
        # # head.transformer.decoder
        # # head.transformer.refine_decoder

        # macs, params = profile(model.module, inputs=(input_tensor))

        # flops = macs * 2  # 1 MAC = 2 FLOPs
        # gflops = flops / 1e9
        # param_mb = params * 4 / (1024 ** 2)  # 参数数 × 4 字节（FP32）换算成 MB
        # params = params / 1e6

        # print(f"GFLOPs: {gflops:.2f}, Parameters: {param_mb:.2f} MB, Parameters_num: {params}")
